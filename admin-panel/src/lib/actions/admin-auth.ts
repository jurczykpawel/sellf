...
/** Context for admin auth -- includes admin client for data queries. */
interface AdminDataContext {
  user: User;
  supabase: SupabaseClient;
  role: 'platform_admin';
  /** Service-role admin client for data queries. */
...
/**
 * Wraps a server action with admin authentication and provides a service-role data client.

 ...
 *
 * If admin edits `recurring_price`, `currency`, `billing_interval`, or
 * `billing_interval_count` after first checkout, the persisted Price no longer
 * matches; a new Price is created and the column is updated. Old Stripe Prices
 * are deactivated (Stripe doesn't support delete on prices used by subs).
 *
...
function priceMatchesProduct(stripePrice: Stripe.Price, product: ProductForPrice): boolean {

...
/** Base URL of the Sellf documentation site (Starlight on Cloudflare Pages). */
export const SELLF_DOCS_URL = 'https://docs.sellf.app';

/** One-click deploy URL for Vercel — keep in sync with README badge.
 *  STRIPE_WEBHOOK_SECRET intentionally omitted — webhook is registered
 *  by the admin from /dashboard/settings after the first signup, which
 *  stores the signing secret encrypted in the DB. */
...
/** Consent cookie name (cookieconsent v3). Distinct from the old consent lib's legacy `sellf_consent` so a host-only leftover can't shadow it. */

...
export default function WebhookFailuresPanel({ refreshTrigger, onRefresh }: WebhookFailuresPanelProps) {
  ...
  return (
    <div className="bg-sf-danger-soft border border-sf-danger/20 overflow-hidden mb-8 animate-in fade-in slide-in-from-top-4 duration-500">
      <div className="px-6 py-4 border-b border-sf-danger/20 flex justify-between items-center bg-sf-danger-soft">
        <div className="flex items-center space-x-2">
          ...
          <h3 className="text-lg font-bold text-sf-danger">
            {t('failuresTitle')}
          </h3>
        </div>
        <button 
          onClick={fetchFailures}
          className="text-sf-danger hover:opacity-80 text-sm flex items-center transition-colors"
        >

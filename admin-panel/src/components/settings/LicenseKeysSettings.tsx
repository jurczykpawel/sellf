...
export default function LicenseKeysSettings() {
  ...
  const handleUpload = async () => {
    ...
    try {
      ...
      if (res.success) {
        ...
        toast.error(res.error || t('actionError'));
        // Refresh in case the prior key was deactivated before the error.
        await load();
      }
    } finally {
      setWorking(false);
  ...
  return (
    <>
        ...
        <div className="bg-sf-accent-soft px-6 py-4 border-b border-sf-border-accent">
          <div className="flex items-center gap-3">
            ...
            <button
              ...
              className="text-xs text-sf-accent hover:underline font-medium flex-shrink-0"
            >
              {t('howToUse')}
            </button>
          </div>
      ...
      <LicenseHowToUseModal
        isOpen={showHowTo}
        onClose={() => setShowHowTo(false)}
        jwksUrl={info?.jwksUrl ?? null}
      />
    </>
  );
}

...
function readExistingValue(existingEnv: string | undefined, key: string): string | null {
...
function generateEnvContent(
  ...
  // Preserve an existing binding secret across reruns so in-flight checkout
  // sessions on a re-setup instance stay valid. Generate a fresh one only
  // when none is present.
  const bindingSecret =
    readExistingValue(existingEnv, 'CHECKOUT_BINDING_SECRET') ||
    randomBytes(32).toString('base64');

  // Same preservation logic for the loginwall handoff secret — rotating
  // invalidates in-flight tokens and that should be intentional, not a
  // re-setup side effect.
  const loginwallSecret =
    readExistingValue(existingEnv, 'LOGINWALL_SECRET') ||
    randomBytes(32).toString('hex');

  // Same preservation logic for the cron job secret — rotating it locks out
  // any external scheduler (cron-job.org, crontab) until the Bearer token is
  // updated there too. Generate one on first setup so /api/cron is callable
  // by an authorized caller out of the box.
  const cronSecret =
    readExistingValue(existingEnv, 'CRON_SECRET') ||
    randomBytes(32).toString('base64');
...
# ===========================================
# CHECKOUT BINDING SECRET (auto-generated)
# Rotating invalidates in-flight checkout sessions — only do it during
# incident response, not casually.
# ===========================================
CHECKOUT_BINDING_SECRET=${bindingSecret}

# ===========================================
# LOGINWALL SECRET (auto-generated)
# HMAC key for the per-product login-wall handoff token. Rotating
# invalidates in-flight loginwall tokens — visitors transparently get
# a fresh one via /loginwall/protect.
# ===========================================
LOGINWALL_SECRET=${loginwallSecret}

# ===========================================
# CRON SECRET (auto-generated)
# Bearer token for /api/cron. Required for scheduled jobs (access-expired
# webhooks, webhook log cleanup). Configure your scheduler (cron-job.org
# or crontab) to hit /api/cron with Authorization: Bearer <this value>.
# ===========================================
CRON_SECRET=${cronSecret}
`;

  if (existingEnv) {
    // Remove existing managed blocks before re-appending the fresh values.
    const cleanedEnv = existingEnv
      ...
      .replace(/CHECKOUT_BINDING_SECRET=.*\n?/g, '')
      .replace(/LOGINWALL_SECRET=.*\n?/g, '')
      .replace(/CRON_SECRET=.*\n?/g, '')
      .trim();

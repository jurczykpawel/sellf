...
-- =====================================================
-- MARKETPLACE: DEMO SELLER STORES
-- =====================================================
-- Provisions two seller stores with products to demonstrate
-- multi-tenant marketplace functionality.
-- seller_main (Platform Owner) is created by migration, not seed.
--
-- Store 1: "Kowalski Digital" — Polish e-commerce educator
-- Store 2: "Creative Studio" — Design & branding agency
...
BEGIN
  -- =====================================================
  -- STORE 1: Kowalski Digital
  -- =====================================================
  SELECT public.provision_seller_schema(
    'kowalski-digital',
    'Kowalski Digital'
  ) INTO v_kowalski_id;
  ...
  (
    ...
    'kurs-ecommerce',
    'Kompleksowy kurs budowania sklepu internetowego.',
    E'## Naucz się sprzedawać online\n\nOd wyboru platformy po pierwsze zamówienie. Kurs oparty na **prawdziwych case study** polskich sklepów.\n\n### Co zawiera?\n\n- 🎥 **40 lekcji wideo** (12h materiału)\n- 📋 Checklisty i szablony\n- 💬 Dostęp do zamkniętej grupy\n\n> "Dzięki temu kursowi otworzyłem swój sklep w 2 tygodnie" — Marek, absolwent',
    '🛒',
    199.00,
    'PLN',
    23,
    true,
    '[{"title": "Zawartość kursu", "items": ["40 lekcji wideo (12h)", "Checklisty wdrożeniowe", "Szablony opisów produktów", "Grupa wsparcia"]}, {"title": "Bonusy", "items": ["Kalkulator marży", "Lista 50 dostawców", "Konsultacja 15 min"]}]'::jsonb,
    true,
    true,
    true
  ),
  (
    ...
    'szablon-sklepu',
    'Gotowy szablon sklepu z konfiguratorem.',
    E'## Profesjonalny szablon e-commerce\n\nResponsywny, zoptymalizowany pod SEO, gotowy do uruchomienia.\n\n- 🎨 3 warianty kolorystyczne\n- 📱 Mobile-first design\n- ⚡ Core Web Vitals w zieleni',
    '🎨',
    79.00,
    'PLN',
    23,
    true,
    '[{"title": "Zawiera", "items": ["3 warianty kolorystyczne", "Responsywny layout", "SEO-ready struktura", "Instrukcja instalacji"]}]'::jsonb,
    true,
    false,
    true
  ),
  (
    'Konsultacja 1:1',
    'konsultacja',
    'Godzinna konsultacja e-commerce z ekspertem.',
    E'## Indywidualna konsultacja\n\nOmówimy Twój sklep, strategię i plan działania.\n\n- 📞 60 min rozmowy video\n- 📝 Notatki po spotkaniu\n- 🎯 Konkretny plan działania',
    '💬',
    299.00,
    'PLN',
    23,
    true,
    '[{"title": "Co otrzymasz", "items": ["60 min konsultacji video", "Analiza Twojego sklepu", "Plan działania (PDF)", "Follow-up email po 7 dniach"]}]'::jsonb,
    true,
    true,
    true
  ),
  (
    'Pakiet Start',
    'pakiet-start',
    'Kurs + szablon + konsultacja w pakiecie ze zniżką.',
    E'## Wszystko czego potrzebujesz\n\nKompletny pakiet dla nowych sprzedawców. Oszczędzasz 30% w porównaniu do zakupu osobno.',
    '🎁',
    399.00,
    'PLN',
    23,
    true,
    '[{"title": "W pakiecie", "items": ["Kurs E-commerce od Zera", "Szablon Sklepu Premium", "Konsultacja 1:1 (60 min)", "3 miesiące wsparcia email"]}]'::jsonb,
    true,
    true,
    true
  );

  -- Order bump: buying "Kurs E-commerce" → offer "Szablon Sklepu" at discount
  INSERT INTO seller_kowalski_digital.order_bumps (
    main_product_id, bump_product_id, bump_price,
    bump_title, bump_description, is_active, display_order
  ) VALUES (
    (SELECT id FROM seller_kowalski_digital.products WHERE slug = 'kurs-ecommerce'),
    (SELECT id FROM seller_kowalski_digital.products WHERE slug = 'szablon-sklepu'),
    49.00,
    '🎨 Dodaj Szablon Sklepu za jedyne 49 zł!',
    'Gotowy szablon premium wart 79 zł. Tylko teraz 38% taniej przy zakupie kursu!',
    true,
    1
  );

  -- Shop config for Kowalski
  INSERT INTO seller_kowalski_digital.shop_config (
    default_currency, shop_name, tax_rate, tax_mode, stripe_tax_rate_cache,
  ...
  INSERT INTO seller_kowalski_digital.categories (name, slug, description) VALUES
    ...
    ('Narzędzia', 'narzedzia', 'Szablony i narzędzia');

  INSERT INTO seller_kowalski_digital.tags (name, slug) VALUES
    ('E-commerce', 'ecommerce'),
    ('Bestseller', 'bestseller'),
    ('Nowość', 'nowosc');

  INSERT INTO seller_kowalski_digital.product_categories (product_id, category_id) VALUES
    ((SELECT id FROM seller_kowalski_digital.products WHERE slug = 'kurs-ecommerce'),
     (SELECT id FROM seller_kowalski_digital.categories WHERE slug = 'kursy')),
    ((SELECT id FROM seller_kowalski_digital.products WHERE slug = 'szablon-sklepu'),
     (SELECT id FROM seller_kowalski_digital.categories WHERE slug = 'narzedzia')),
    ((SELECT id FROM seller_kowalski_digital.products WHERE slug = 'konsultacja'),
     (SELECT id FROM seller_kowalski_digital.categories WHERE slug = 'kursy'));

  INSERT INTO seller_kowalski_digital.product_tags (product_id, tag_id) VALUES
    ((SELECT id FROM seller_kowalski_digital.products WHERE slug = 'kurs-ecommerce'),
     (SELECT id FROM seller_kowalski_digital.tags WHERE slug = 'bestseller')),
    ((SELECT id FROM seller_kowalski_digital.products WHERE slug = 'kurs-ecommerce'),
     (SELECT id FROM seller_kowalski_digital.tags WHERE slug = 'ecommerce')),
    ((SELECT id FROM seller_kowalski_digital.products WHERE slug = 'pakiet-start'),
     (SELECT id FROM seller_kowalski_digital.tags WHERE slug = 'nowosc'));

  -- =====================================================
  -- STORE 2: Creative Studio
  -- =====================================================
  SELECT public.provision_seller_schema(
    'creative-studio',
    'Creative Studio'
  ...
  (
    ...
    E'## Professional Logo Design\n\nWe create **distinctive, memorable logos** that capture your brand essence.\n\n### Process\n\n1. 📋 Brand questionnaire\n2. 🎨 3 initial concepts\n3. ✏️ Unlimited revisions on chosen concept\n4. 📦 Final files in all formats\n\n> "They nailed our brand identity on the first try" — TechStartup.io',
    '✏️',
    499.00,
    'PLN',
    23,
    true,
    '[{"title": "Deliverables", "items": ["3 logo concepts", "Unlimited revisions", "Vector files (AI, SVG, EPS)", "PNG/JPG in all sizes", "Brand color palette"]}, {"title": "Timeline", "items": ["First concepts: 3-5 days", "Revisions: 1-2 days each", "Final delivery: 7-10 days"]}]'::jsonb,
    true,
    true,
    true
  ),
  (
    ...
    E'## Complete Brand Identity\n\nEverything you need for a **consistent, professional brand presence**.\n\n- 🎨 Logo suite (primary + variations)\n- 📐 Typography system\n- 🎨 Color palette with codes\n- 📄 Stationery design\n- 📖 Brand guidelines PDF (30+ pages)',
    '🎨',
    1299.00,
    'PLN',
    23,
    true,
    '[{"title": "Full package", "items": ["Logo suite (5 variations)", "Typography system", "Color palette (primary + secondary)", "Business card design", "Letterhead + envelope", "Brand guidelines (30+ pages)"]}, {"title": "Bonus", "items": ["Social media templates (5)", "Email signature", "Favicon + app icon"]}]'::jsonb,
    true,
    true,
    true
  ),
  (
    ...
    'social-templates',
    'Pack of 30 customizable social media templates.',
    E'## Social Media Template Pack\n\n30 professionally designed templates for Instagram, Facebook, and LinkedIn.\n\n- 📐 Canva + Figma formats\n- 🎨 Easily customizable\n- 📱 Stories + posts + covers',
    '📱',
    149.00,
    'PLN',
    23,
    true,
    '[{"title": "Templates included", "items": ["10 Instagram post templates", "10 Instagram story templates", "5 Facebook cover designs", "5 LinkedIn banner designs"]}, {"title": "Formats", "items": ["Canva (editable)", "Figma (editable)", "PNG exports"]}]'::jsonb,
    true,
    false,
    true
  );

  -- Order bump: buying "Logo Design" → offer "Social Media Templates" at discount
  INSERT INTO seller_creative_studio.order_bumps (
    main_product_id, bump_product_id, bump_price,
    bump_title, bump_description, is_active, display_order
  ) VALUES (
    (SELECT id FROM seller_creative_studio.products WHERE slug = 'logo-design'),
    (SELECT id FROM seller_creative_studio.products WHERE slug = 'social-templates'),
    99.00,
    '📱 Dodaj Social Media Templates za 99 zł!',
    'Szablony social media dopasowane do Twojego nowego logo. Warte 149 zł — oszczędzasz 33%!',
    true,
    1
  );

  -- OTO: after buying "Logo Design" → offer "Brand Identity Kit" at discount
  INSERT INTO seller_creative_studio.oto_offers (
    source_product_id, oto_product_id,
    discount_type, discount_value, duration_minutes,
    is_active, display_order
  ) VALUES (
    (SELECT id FROM seller_creative_studio.products WHERE slug = 'logo-design'),
    (SELECT id FROM seller_creative_studio.products WHERE slug = 'brand-identity'),
    'percentage',
    25,
    20,
    true,
  ...
  -- Shop config for Creative Studio
  INSERT INTO seller_creative_studio.shop_config (
    default_currency, shop_name, tax_rate, tax_mode, stripe_tax_rate_cache,
    font_family, custom_settings
  ) VALUES (
    'PLN',
    ...
    '{}'::jsonb,
    'system',
    '{"accent_color": "#7c3aed"}'::jsonb
  );

  -- Categories for Creative Studio
  INSERT INTO seller_creative_studio.categories (name, slug, description) VALUES
    ('Design', 'design', 'Design services and products'),
    ('Templates', 'templates', 'Ready-to-use design templates');

  INSERT INTO seller_creative_studio.tags (name, slug) VALUES
    ('Branding', 'branding'),
    ('Popular', 'popular');

  INSERT INTO seller_creative_studio.product_categories (product_id, category_id) VALUES
    ((SELECT id FROM seller_creative_studio.products WHERE slug = 'logo-design'),
     (SELECT id FROM seller_creative_studio.categories WHERE slug = 'design')),
    ((SELECT id FROM seller_creative_studio.products WHERE slug = 'brand-identity'),
     (SELECT id FROM seller_creative_studio.categories WHERE slug = 'design')),
    ((SELECT id FROM seller_creative_studio.products WHERE slug = 'social-templates'),
     (SELECT id FROM seller_creative_studio.categories WHERE slug = 'templates'));

  INSERT INTO seller_creative_studio.product_tags (product_id, tag_id) VALUES
    ((SELECT id FROM seller_creative_studio.products WHERE slug = 'logo-design'),
     (SELECT id FROM seller_creative_studio.tags WHERE slug = 'popular')),
    ((SELECT id FROM seller_creative_studio.products WHERE slug = 'brand-identity'),
     (SELECT id FROM seller_creative_studio.tags WHERE slug = 'branding'));

  -- =====================================================
  -- KOWALSKI: Free product (for grant-access testing)
  -- =====================================================
  INSERT INTO seller_kowalski_digital.products (
    name, slug, description, long_description, icon, price, currency,
    vat_rate, price_includes_vat, features, is_active, is_featured, is_listed
    ...
    'darmowy-poradnik',
    'Bezplatny poradnik dla poczatkujacych sprzedawcow.',
    E'## Darmowy Poradnik\n\nPobierz za darmo i zacznij sprzedawac online.\n\n- 📖 30 stron praktycznych porad\n- ✅ Checklista przed startem\n- 📧 Newsletter z aktualizacjami',
    '📚',
    0,
    'PLN',
    23,
    true,
    '[{"title": "Zawiera", "items": ["30 stron porad", "Checklista startowa", "Newsletter"]}]'::jsonb,
    true,
    false,
    true
  );
  ...
  -- KOWALSKI: Coupons
  -- =====================================================
  INSERT INTO seller_kowalski_digital.coupons (
    code, name, discount_type, discount_value, usage_limit_global, is_active
  ) VALUES
  (
    'KOWALSKI20',
    'Kowalski 20% rabat',
  ...
  -- =====================================================
  -- CREATIVE STUDIO: Free product + coupon
  -- =====================================================
  INSERT INTO seller_creative_studio.products (
    name, slug, description, long_description, icon, price, currency,
    vat_rate, price_includes_vat, features, is_active, is_featured, is_listed
    ...
    'free-brand-checklist',
    'A free checklist to audit your brand identity.',
    E'## Brand Audit Checklist\n\nDownload for free. Evaluate your brand across 50 dimensions.\n\n- ✅ 50-point brand audit\n- 📊 Scoring template\n- 📋 Action plan worksheet',
    '✅',
    0,
    'PLN',
    23,
    true,
    '[{"title": "Includes", "items": ["50-point checklist", "Scoring template", "Action plan"]}]'::jsonb,
    true,
    false,
    true
  );

  INSERT INTO seller_creative_studio.coupons (
    code, name, discount_type, discount_value, usage_limit_global, is_active
  ) VALUES (
    'DESIGN10',
    'Design 10% discount',
  ...
  RAISE NOTICE 'Marketplace seed complete: Kowalski Digital (%), Creative Studio (%)',
    v_kowalski_id, v_creative_id;
END $$;
...
INSERT INTO auth.users (
  ...
  '00000000-0000-0000-0000-000000000000',
  'eeeeeeee-3333-4000-a000-000000000003',
  'authenticated', 'authenticated',
...
VALUES (
  'eeeeeeee-3333-4000-a000-000000000003',
  'eeeeeeee-3333-4000-a000-000000000003',
  jsonb_build_object('sub', 'eeeeeeee-3333-4000-a000-000000000003', 'email', 'buyer@demo.sellf.app'),
  'email', NOW(), NOW(), NOW()
) ON CONFLICT DO NOTHING;

-- Grant buyer access to products in both seller schemas
-- Wrapped in DO block because seller schemas are created dynamically
DO $$
BEGIN
  INSERT INTO seller_kowalski_digital.user_product_access (user_id, product_id, access_granted_at)
  VALUES (
    'eeeeeeee-3333-4000-a000-000000000003',
    (SELECT id FROM seller_kowalski_digital.products WHERE slug = 'kurs-ecommerce'),
    NOW() - INTERVAL '3 days'
  ...
  INSERT INTO seller_kowalski_digital.payment_transactions (
    session_id, user_id, product_id, customer_email, amount, currency, stripe_payment_intent_id, status
  ) VALUES (
    'pi_demo_kowalski_buyer', 'eeeeeeee-3333-4000-a000-000000000003',
    (SELECT id FROM seller_kowalski_digital.products WHERE slug = 'kurs-ecommerce'),
    'buyer@demo.sellf.app', 19900, 'PLN', 'pi_demo_kowalski_buyer', 'completed'
  );

  INSERT INTO seller_creative_studio.user_product_access (user_id, product_id, access_granted_at)
  VALUES (
    'eeeeeeee-3333-4000-a000-000000000003',
    (SELECT id FROM seller_creative_studio.products WHERE slug = 'logo-design'),
    NOW() - INTERVAL '1 day'
  ...
  INSERT INTO seller_creative_studio.payment_transactions (
    session_id, user_id, product_id, customer_email, amount, currency, stripe_payment_intent_id, status
  ) VALUES (
    'pi_demo_creative_buyer', 'eeeeeeee-3333-4000-a000-000000000003',
    (SELECT id FROM seller_creative_studio.products WHERE slug = 'logo-design'),
    'buyer@demo.sellf.app', 49900, 'PLN', 'pi_demo_creative_buyer', 'completed'
  );
END $$;

-- =====================================================
-- MARKETPLACE: Manual testing guide
-- =====================================================
-- After db reset, start dev server: cd admin-panel && bun run dev
--
-- DEMO ACCOUNTS (ready to use, no registration needed):
--   demo@sellf.app / demo123         → Platform admin (full access)
--   kowalski@demo.sellf.app / demo1234 → Seller admin (Kowalski Digital)
--   creative@demo.sellf.app / demo1234 → Seller admin (Creative Studio)
--   buyer@demo.sellf.app / demo1234    → Buyer (has purchases from both sellers)
--
-- BROWSE AS BUYER:
--   /s/kowalski-digital              → Kowalski Digital storefront
--   /s/kowalski-digital/kurs-ecommerce → Product page (199 PLN)
--   /s/kowalski-digital/darmowy-poradnik → Free product (grant-access flow)
--   /s/creative-studio               → Creative Studio storefront
--   /s/creative-studio/logo-design   → Product page (499 PLN)
--   /s/creative-studio/free-brand-checklist → Free product
--
-- SELLER ADMIN (login as kowalski@demo.sellf.app):
--   /dashboard                       → Seller dashboard (only Kowalski products)
--   /dashboard/products              → Manage products
--   /dashboard/settings              → Shop settings
--   Hidden tabs: Users, API Keys, Integrations (platform-only)
--
-- COUPONS:
--   Kowalski: KOWALSKI20 (20% off), ECOMMERCE50 (50 PLN off)
--   Creative: DESIGN10 (10% off)
--
-- ORDER BUMPS:
--   Kowalski: kurs-ecommerce → szablon-sklepu at 49 PLN
--   Creative: logo-design → social-templates at 99 PLN
--
-- OTO (One-Time Offers):
--   Creative: after logo-design → brand-identity at 25% off (20 min)
--
-- NOTE: Checkout requires Stripe keys in .env.local. Seller checkout also
-- requires stripe_account_id on the seller (Stripe Connect). To enable:
--   UPDATE sellers SET stripe_account_id='acct_test', stripe_onboarding_complete=true
--   WHERE slug='kowalski_digital';
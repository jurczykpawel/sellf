...
CREATE TABLE IF NOT EXISTS seller_main.oto_offers (
  id UUID DEFAULT gen_random_uuid() PRIMARY KEY,

  -- Product relationships
  source_product_id UUID REFERENCES seller_main.products(id) ON DELETE CASCADE NOT NULL,
  oto_product_id UUID REFERENCES seller_main.products(id) ON DELETE CASCADE NOT NULL,
...
COMMENT ON TABLE seller_main.oto_offers IS 'OTO (One-Time Offer) configurations - defines which product to offer after purchasing another';
COMMENT ON COLUMN seller_main.oto_offers.source_product_id IS 'The product that triggers the OTO after purchase';
COMMENT ON COLUMN seller_main.oto_offers.oto_product_id IS 'The product offered as OTO with a discount';
COMMENT ON COLUMN seller_main.oto_offers.duration_minutes IS 'How long the OTO coupon is valid (default: 15 minutes, max: 24 hours)';
...
ADD COLUMN IF NOT EXISTS oto_offer_id UUID REFERENCES seller_main.oto_offers(id) ON DELETE SET NULL,
ADD COLUMN IF NOT EXISTS source_transaction_id UUID REFERENCES seller_main.payment_transactions(id) ON DELETE SET NULL;
...
COMMENT ON COLUMN seller_main.coupons.is_oto_coupon IS 'True if this coupon was auto-generated by the OTO system';
COMMENT ON COLUMN seller_main.coupons.oto_offer_id IS 'Reference to the OTO offer that generated this coupon';
COMMENT ON COLUMN seller_main.coupons.source_transaction_id IS 'The transaction that triggered OTO coupon generation';
...
CREATE INDEX IF NOT EXISTS idx_oto_offers_source_product
  ON seller_main.oto_offers(source_product_id, is_active, display_order);
CREATE INDEX IF NOT EXISTS idx_oto_offers_oto_product
  ON seller_main.oto_offers(oto_product_id);

-- Coupons OTO indexes
CREATE INDEX IF NOT EXISTS idx_coupons_oto
  ON seller_main.coupons(is_oto_coupon) WHERE is_oto_coupon = true;
CREATE INDEX IF NOT EXISTS idx_coupons_oto_expires
  ON seller_main.coupons(expires_at) WHERE is_oto_coupon = true;
CREATE INDEX IF NOT EXISTS idx_coupons_source_transaction
  ON seller_main.coupons(source_transaction_id) WHERE source_transaction_id IS NOT NULL;
...
CREATE UNIQUE INDEX IF NOT EXISTS idx_coupons_oto_transaction_unique
  ON seller_main.coupons(source_transaction_id)
...
CREATE OR REPLACE FUNCTION seller_main.get_oto_coupon_info(
...
BEGIN
  ...
  INTO coupon_record
  FROM seller_main.coupons c
  LEFT JOIN seller_main.oto_offers o ON c.oto_offer_id = o.id
  LEFT JOIN seller_main.products p ON o.oto_product_id = p.id
...
CREATE OR REPLACE FUNCTION seller_main.cleanup_expired_oto_coupons()
RETURNS INTEGER AS $$
DECLARE
...
BEGIN
  ...
  -- 3. Were never used
  DELETE FROM seller_main.coupons
...
CREATE OR REPLACE FUNCTION seller_main.admin_get_product_oto_offer(
...
BEGIN
  ...
  INTO oto_record
  FROM seller_main.oto_offers o
  INNER JOIN seller_main.products p ON p.id = o.oto_product_id
...
CREATE OR REPLACE FUNCTION seller_main.admin_save_oto_offer(
  source_product_id_param UUID,
...
CREATE OR REPLACE FUNCTION seller_main.admin_delete_oto_offer(
  source_product_id_param UUID
) RETURNS JSONB AS $$
DECLARE
  deleted_count INTEGER;
BEGIN
  ...
  DELETE FROM seller_main.oto_offers
...
CREATE TRIGGER trigger_update_oto_offers_updated_at
  BEFORE UPDATE ON seller_main.oto_offers
...
ALTER TABLE seller_main.oto_offers ENABLE ROW LEVEL SECURITY;
...
CREATE POLICY "Public can view active oto_offers" ON seller_main.oto_offers
...
COMMENT ON FUNCTION seller_main.generate_oto_coupon IS 'Generates a time-limited, email-bound coupon for OTO after purchase';
COMMENT ON FUNCTION seller_main.get_oto_coupon_info IS 'Returns OTO coupon details for frontend timer display';
COMMENT ON FUNCTION seller_main.cleanup_expired_oto_coupons IS 'Cleanup function for expired OTO coupons - call via pg_cron or external trigger';
COMMENT ON FUNCTION seller_main.admin_get_product_oto_offer IS 'Get OTO configuration for a product (admin only)';
COMMENT ON FUNCTION seller_main.admin_save_oto_offer IS 'Create or update OTO offer for a product (admin only)';
COMMENT ON FUNCTION seller_main.admin_delete_oto_offer IS 'Delete OTO offer for a product (admin only)';

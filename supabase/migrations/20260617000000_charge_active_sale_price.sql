-- Charge the active sale price (Omnibus), not the regular price.
--
-- BUG: sale_price was wired only into the display/Omnibus layer. The actual
-- charge pipeline (client total, Stripe line items) plus this completion
-- validator all used product.price. With no coupon the validator REJECTS any
-- amount != full price, so even after the app charges the sale price the DB
-- would reject the purchase with "Amount mismatch".
--
-- FIX: compute the effective unit price (sale price when is_sale_price_active,
-- else regular price) and validate against it. With no coupon, accept any
-- amount in [effective, full]: an active sale lowers the floor to the sale
-- price; overpaying up to the full price is harmless and this also tolerates a
-- sale flipping state between charge time and the (possibly delayed) webhook.
-- The recorded main line-item price becomes the effective price too.
--
-- This redefines the impl body (the wrapper added in
-- 20260610112649 / 20260515180000 is unchanged and keeps delegating here).
...
BEGIN
  ...
  SELECT id, name, auto_grant_duration_days, price, currency, allow_custom_price, custom_price_min,
         sale_price, sale_price_until, sale_quantity_limit, sale_quantity_sold INTO product_record
  ...
  IF product_record.price IS NOT NULL THEN
    ...
    BEGIN
      ...
      ELSIF coupon_id_param IS NULL THEN
        IF amount_total < (effective_total * 100) OR amount_total > (expected_total * 100) THEN

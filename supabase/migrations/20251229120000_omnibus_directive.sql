...
COMMENT ON COLUMN public.products.omnibus_exempt IS
...
COMMENT ON COLUMN public.products.sale_price_until IS
...
CREATE INDEX IF NOT EXISTS idx_product_price_history_current
  ON public.product_price_history(product_id, effective_until) WHERE effective_until IS NULL;
...
COMMENT ON COLUMN public.shop_config.omnibus_enabled IS
...
COMMENT ON FUNCTION public.log_product_price_change() IS
...
COMMENT ON FUNCTION public.cleanup_old_price_history() IS
...
DROP TRIGGER IF EXISTS cleanup_price_history_trigger ON public.product_price_history;

CREATE TRIGGER cleanup_price_history_trigger
  AFTER INSERT ON public.product_price_history
  FOR EACH STATEMENT
  EXECUTE FUNCTION public.cleanup_old_price_history();

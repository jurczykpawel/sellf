...
CREATE OR REPLACE FUNCTION public.update_payment_method_config_timestamp()
RETURNS TRIGGER
LANGUAGE plpgsql
SET search_path = ''
AS $$
...
CREATE TRIGGER payment_method_config_updated_at
  ...
  FOR EACH ROW
  EXECUTE FUNCTION public.update_payment_method_config_timestamp();

-- Trigger to protect created_at from modification
CREATE OR REPLACE FUNCTION public.protect_payment_method_config_created_at()
RETURNS TRIGGER
LANGUAGE plpgsql
SET search_path = ''
AS $$
...
CREATE TRIGGER payment_method_config_protect_created_at
  BEFORE UPDATE ON public.payment_method_config
  FOR EACH ROW
  EXECUTE FUNCTION public.protect_payment_method_config_created_at();
...
REVOKE ALL ON public.payment_method_config FROM anon, authenticated;
GRANT SELECT, UPDATE ON public.payment_method_config TO authenticated;

...
ALTER TABLE public.products ALTER COLUMN vat_rate SET DEFAULT NULL;

-- Set existing products with hardcoded 23% to NULL (will use shop_config default on next edit)
UPDATE public.products SET vat_rate = NULL WHERE vat_rate = 23.00;
...
UPDATE public.shop_config SET tax_mode = 'local' WHERE automatic_tax_enabled = false;
UPDATE public.shop_config SET tax_mode = 'stripe_tax' WHERE automatic_tax_enabled IS NULL OR automatic_tax_enabled = true;
